...
class MagicTodoOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        if user_input is not None:
            # Store the user input and check next steps
            self.user_input = user_input
            enable_smart_lists = user_input.get(CONF_ENABLE_SMART_LISTS, False)
            auto_clear_entities = user_input.get("auto_clear_entities", [])

            if enable_smart_lists:
                # Proceed to smart list configuration step
                return await self.async_step_smart_list_config()
            elif auto_clear_entities:
                # Proceed to auto-clear configuration step
                return await self.async_step_auto_clear_config()
            else:
                # No additional configuration needed, convert and finish
                converted_options = self._convert_selections_to_entity_options(
                    user_input
                )
        ...
        todo_entity_ids = [
            eid
            for eid in self.hass.states.async_entity_ids(TODO_DOMAIN)
            if self.hass.states.get(eid)
            and self.hass.states.get(eid).state != "unavailable"
        ]
        ...
        valid_selections = self._filter_valid_entities(
            current_selections, todo_entity_ids
        )
        LOGGER.debug("Valid selections: %s", valid_selections)

        # Build schema with entity selectors for each feature
        schema_dict = {
            # Smart Lists - First option
            vol.Optional(
                CONF_ENABLE_SMART_LISTS,
                default=valid_selections.get(CONF_ENABLE_SMART_LISTS, False),
            ...
            vol.Optional(
                ...
                selector.EntitySelectorConfig(domain=TODO_DOMAIN, multiple=True)
            ),
            # Auto Sort
            vol.Optional(
                "auto_sort_entities",
        ...
        return self.async_show_form(
            ...
            description_placeholders={"todo_count": str(len(todo_entity_ids))},
        )
    ...
    async def async_step_auto_clear_config(
        ...
        for entity_id in auto_clear_entities:
            ...
            if state:
                ...
                entity_descriptions[f"{entity_key}_clear_days"] = (
                    f"Days to keep completed tasks in {friendly_name}"
                )
    ...
    async def async_step_smart_list_config(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
        """Configure smart list assignments."""
        if user_input is not None:
            # Merge smart list configuration with the original input
            merged_input = self.user_input.copy()
            merged_input.update(user_input)

            # Check if we still need auto-clear configuration
            auto_clear_entities = merged_input.get("auto_clear_entities", [])
            if auto_clear_entities:
                # Store the updated input and proceed to auto-clear config
                self.user_input = merged_input
                return await self.async_step_auto_clear_config()
            else:
                # Convert and finish
                converted_options = self._convert_selections_to_entity_options(
        ...
        # Get available todo entities for smart list selection
        todo_entity_ids = [
            eid
            for eid in self.hass.states.async_entity_ids(TODO_DOMAIN)
            if self.hass.states.get(eid)
        ...
        if not todo_entity_ids:
            # No entities available, skip smart list config
            converted_options = self._convert_selections_to_entity_options(
                self.user_input
            )
            return self.async_create_entry(title="", data=converted_options)

        # Get current smart list settings
        current_smart_list_settings = self._get_current_smart_list_settings()

        # Create options for entity selectors (including "None" option)
        entity_options = [{"value": "", "label": "None"}]
        for entity_id in todo_entity_ids:
            state = self.hass.states.get(entity_id)
            if state:
                friendly_name = state.attributes.get("friendly_name", entity_id)
                entity_options.append({"value": entity_id, "label": friendly_name})

        # Build schema for smart list configuration
        schema_dict = {
            vol.Optional(
                CONF_DAILY_LIST,
                default=current_smart_list_settings.get(CONF_DAILY_LIST, ""),
            ): selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=entity_options, mode=selector.SelectSelectorMode.DROPDOWN
                )
            ),
            vol.Optional(
                CONF_WEEKLY_LIST,
                default=current_smart_list_settings.get(CONF_WEEKLY_LIST, ""),
            ): selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=entity_options, mode=selector.SelectSelectorMode.DROPDOWN
                )
            ...
            vol.Optional(
                CONF_MONTHLY_LIST,
                default=current_smart_list_settings.get(CONF_MONTHLY_LIST, ""),
            ): selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=entity_options, mode=selector.SelectSelectorMode.DROPDOWN
                )
            ...
            vol.Required(
                CONF_FALLBACK_LIST,
                default=current_smart_list_settings.get(CONF_FALLBACK_LIST, ""),
            ): selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=[
                        opt for opt in entity_options if opt["value"] != ""
                    ],  # Exclude "None" for fallback
        ...
        return self.async_show_form(
            step_id="smart_list_config",
            data_schema=vol.Schema(schema_dict),
            description_placeholders={"entity_count": str(len(todo_entity_ids))},
        )
    ...
    def _get_current_selections_from_options(self) -> dict[str, Any]:
        ...
        selections = {
            ...
            "process_recurring_entities": [],
            "auto_clear_entities": [],
            CONF_ENABLE_SMART_LISTS: current_options.get(
                CONF_ENABLE_SMART_LISTS, False
            ),
        }
        ...
        for key, value in current_options.items():
            if "_auto_due_parsing" in key and value:
                entity_id = self._key_to_entity_id(
                    key.replace("_auto_due_parsing", ""), all_todo_entities
                )
                if entity_id:
                    selections["auto_due_parsing_entities"].append(entity_id)
            elif "_auto_sort" in key and value:
                entity_id = self._key_to_entity_id(
                    key.replace("_auto_sort", ""), all_todo_entities
                )
                if entity_id:
                    selections["auto_sort_entities"].append(entity_id)
            elif "_process_recurring" in key and value:
                entity_id = self._key_to_entity_id(
                    key.replace("_process_recurring", ""), all_todo_entities
                )
                if entity_id:
                    selections["process_recurring_entities"].append(entity_id)
            elif (
                "_clear_days" in key and isinstance(value, (int, float)) and value >= 0
            ):
                entity_id = self._key_to_entity_id(
                    key.replace("_clear_days", ""), all_todo_entities
                )
                if entity_id:
    ...
    def _get_current_smart_list_settings(self) -> dict[str, Any]:
        """Get current smart list settings from options."""
        current_options = self.config_entry.options
        return {
            CONF_DAILY_LIST: current_options.get(CONF_DAILY_LIST, ""),
            CONF_WEEKLY_LIST: current_options.get(CONF_WEEKLY_LIST, ""),
            CONF_MONTHLY_LIST: current_options.get(CONF_MONTHLY_LIST, ""),
            CONF_FALLBACK_LIST: current_options.get(CONF_FALLBACK_LIST, ""),
            CONF_ENABLE_SMART_LISTS: current_options.get(
                CONF_ENABLE_SMART_LISTS, False
            ),
        }
    ...
    def _filter_valid_entities(
        self, selections: dict[str, Any], valid_entities: list[str]
    ) -> dict[str, Any]:
    ...
    def _convert_selections_to_entity_options(
        self, user_input: dict[str, Any]
    ) -> dict[str, Any]:
        """Convert entity list selections to individual entity options."""
        options = {}

        # First, handle smart list settings (these are global, not per-entity)
        options[CONF_ENABLE_SMART_LISTS] = user_input.get(
            CONF_ENABLE_SMART_LISTS, False
        )
        options[CONF_DAILY_LIST] = user_input.get(CONF_DAILY_LIST, "")
        options[CONF_WEEKLY_LIST] = user_input.get(CONF_WEEKLY_LIST, "")
        options[CONF_MONTHLY_LIST] = user_input.get(CONF_MONTHLY_LIST, "")
        options[CONF_FALLBACK_LIST] = user_input.get(CONF_FALLBACK_LIST, "")
        ...
        for entity_id in all_todo_entities:
            ...
            # Auto due parsing
            options[f"{entity_key}_auto_due_parsing"] = entity_id in user_input.get(
                "auto_due_parsing_entities", []
            )

            # Auto sort
            options[f"{entity_key}_auto_sort"] = entity_id in user_input.get(
                "auto_sort_entities", []
            )

            # Process recurring
            options[f"{entity_key}_process_recurring"] = entity_id in user_input.get(
                "process_recurring_entities", []
            )
